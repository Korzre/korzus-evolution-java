...
public class World {
	...
	public World(String path) {
			try {
				// Depois de colocar a cor em hexadecimal
				// a frente da cor tem que se escrever dois F, "FF".
				...
				for(int xx=0;xx<map.getWidth();xx++) {
					for(int yy=0;yy<map.getHeight();yy++) {
						...
						}else if(pixelAtual == 0xFFFF000C){
							// Enemy
							BufferedImage[] buf = new BufferedImage[2];
							buf[0] =Game.spritesheet.getSprite(9*16, 16, 16, 16);
							buf[1] = Game.spritesheet.getSprite(8*16, 16, 16, 16);
							Enemy en = new Enemy(xx*16,yy*16,16,16,Entity.ENEMY_EN);
							Game.entities.add(en);
							Game.enemies.add(en);
							
						}else if(pixelAtual == 0xFFFFD69E) {
							// Weapon
